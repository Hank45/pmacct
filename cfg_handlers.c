...
int cfg_key_aggregate(char *filename, char *name, char *value_ptr)
{
  ...
  while (count_token = extract_token(&value_ptr, ',')) {
    ...
    else if (!strcmp(count_token, "src_host_lat")) cfg_set_aggregate(filename, value, COUNT_INT_SRC_HOST_LAT, count_token);
    else if (!strcmp(count_token, "src_host_long")) cfg_set_aggregate(filename, value, COUNT_INT_SRC_HOST_LONG, count_token);
    else if (!strcmp(count_token, "dst_host_country")) cfg_set_aggregate(filename, value, COUNT_INT_DST_HOST_COUNTRY, count_token);
    else if (!strcmp(count_token, "dst_host_lat")) cfg_set_aggregate(filename, value, COUNT_INT_DST_HOST_LAT, count_token);
    else if (!strcmp(count_token, "dst_host_long")) cfg_set_aggregate(filename, value, COUNT_INT_DST_HOST_LONG, count_token);
