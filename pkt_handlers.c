...
void evaluate_packet_handlers()
{
  ...
  while (channels_list[index].aggregation) { 
	...
	if (channels_list[index].aggregation_2 & COUNT_SRC_HOST_LAT /* other GeoIP primitives here */) {
      channels_list[index].phandler[primitives] = src_host_geoipv2_lookup_handler;
      primitives++;
	

      if (channels_list[index].aggregation_2 & COUNT_SRC_HOST_LAT) {
        channels_list[index].phandler[primitives] = src_host_lat_geoipv2_handler;
        primitives++;
	
      }
    }
	if (channels_list[index].aggregation_2 & COUNT_SRC_HOST_LONG /* other GeoIP primitives here */) {
      channels_list[index].phandler[primitives] = src_host_geoipv2_lookup_handler;
      primitives++;
	
      if (channels_list[index].aggregation_2 & COUNT_SRC_HOST_LONG) {
        channels_list[index].phandler[primitives] = src_host_long_geoipv2_handler;
        primitives++;
      }
    }
    
	if (channels_list[index].aggregation_2 & COUNT_DST_HOST_LAT /* other GeoIP primitives here */) {
     channels_list[index].phandler[primitives] = dst_host_geoipv2_lookup_handler;
     primitives++;
	

      if (channels_list[index].aggregation_2 & COUNT_DST_HOST_LAT) {
        channels_list[index].phandler[primitives] = dst_host_lat_geoipv2_handler;
        primitives++;
	
      }
   }
	if (channels_list[index].aggregation_2 & COUNT_DST_HOST_LONG /* other GeoIP primitives here */) {
     channels_list[index].phandler[primitives] = dst_host_geoipv2_lookup_handler;
     primitives++;


      if (channels_list[index].aggregation_2 & COUNT_DST_HOST_LONG) {
        channels_list[index].phandler[primitives] = dst_host_long_geoipv2_handler;
        primitives++;
...
void dst_host_long_geoipv2_handler(struct channels_list_entry *chptr, struct packet_ptrs *pptrs, char **data)
{
  struct pkt_data *pdata = (struct pkt_data *) *data;
  MMDB_entry_data_list_s *entry_data_list = NULL;
  int status;
  ...
  if (pptrs->geoipv2_dst.found_entry) {
    MMDB_entry_data_s entry_data;
	status = MMDB_get_value(&pptrs->geoipv2_dst.entry, &entry_data, "location", "longitude", NULL,NULL);
    ...
    if (status != MMDB_SUCCESS && status != MMDB_LOOKUP_PATH_DOES_NOT_MATCH_DATA_ERROR) {
      Log(LOG_WARNING, "WARN ( %s/%s ): dst_host_long_geoipv2_handler(): %s\n", config.name, config.type, MMDB_strerror(status));
    }

    if (entry_data_list != NULL) {
      if (entry_data_list->entry_data.has_data) {
		  int size = PM_LATLONG_T;  
		  
		  snprintf(pdata->primitives.dst_ip_long.str,size,"%f",entry_data_list->entry_data.double_value);
          pdata->primitives.dst_ip_long.str[size] = '\0';
      }
...
void dst_host_lat_geoipv2_handler(struct channels_list_entry *chptr, struct packet_ptrs *pptrs, char **data)
{
  struct pkt_data *pdata = (struct pkt_data *) *data;
  MMDB_entry_data_list_s *entry_data_list = NULL;
  int status;
  ...
  if (pptrs->geoipv2_dst.found_entry) {
    MMDB_entry_data_s entry_data;
	status = MMDB_get_value(&pptrs->geoipv2_dst.entry, &entry_data, "location", "latitude", NULL,NULL);
    ...
    if (status != MMDB_SUCCESS && status != MMDB_LOOKUP_PATH_DOES_NOT_MATCH_DATA_ERROR) {
      Log(LOG_WARNING, "WARN ( %s/%s ): dst_host_lat_geoipv2_handler(): %s\n", config.name, config.type, MMDB_strerror(status));
    }

    if (entry_data_list != NULL) {
      if (entry_data_list->entry_data.has_data) {
		  int size = PM_LATLONG_T;		  
		  snprintf(pdata->primitives.dst_ip_lat.str,size,"%f",entry_data_list->entry_data.double_value);
          pdata->primitives.dst_ip_lat.str[size] = '\0';
		  	}
...
void src_host_long_geoipv2_handler(struct channels_list_entry *chptr, struct packet_ptrs *pptrs, char **data)
{
  struct pkt_data *pdata = (struct pkt_data *) *data;
  MMDB_entry_data_list_s *entry_data_list = NULL;
  int status;
  ...
  if (pptrs->geoipv2_src.found_entry) {
    MMDB_entry_data_s entry_data;
	status = MMDB_get_value(&pptrs->geoipv2_src.entry, &entry_data, "location", "longitude", NULL,NULL);
    ...
    if (status != MMDB_SUCCESS && status != MMDB_LOOKUP_PATH_DOES_NOT_MATCH_DATA_ERROR) {
      Log(LOG_WARNING, "WARN ( %s/%s ): src_host_long_geoipv2_handler(): %s\n", config.name, config.type, MMDB_strerror(status));
    }

    if (entry_data_list != NULL) {
      if (entry_data_list->entry_data.has_data) {
		int size = PM_LATLONG_T;
		  snprintf(pdata->primitives.src_ip_long.str,size,"%f",entry_data_list->entry_data.double_value);
          pdata->primitives.src_ip_long.str[size] = '\0';
      }
...
void src_host_lat_geoipv2_handler(struct channels_list_entry *chptr, struct packet_ptrs *pptrs, char **data)
{
  ...
  if (pptrs->geoipv2_src.found_entry) {
    MMDB_entry_data_s entry_data;
	status = MMDB_get_value(&pptrs->geoipv2_src.entry, &entry_data, "location", "latitude", NULL,NULL);
    ...
    if (entry_data_list != NULL) {
      if (entry_data_list->entry_data.has_data) {
		int size = PM_LATLONG_T;
		  snprintf(pdata->primitives.src_ip_lat.str,size,"%f",entry_data_list->entry_data.double_value);
          pdata->primitives.src_ip_lat.str[size] = '\0';
      }
