		...
		struct extra_primitives *extras)
{
  ...
  if (w2 & COUNT_SRC_HOST_LAT) memcpy(&d1->src_ip_lat, &s1->src_ip_lat, sizeof(d1->src_ip_lat)); 
  if (w2 & COUNT_SRC_HOST_LONG) memcpy(&d1->src_ip_long, &s1->src_ip_long, sizeof(d1->src_ip_long)); 
  if (w2 & COUNT_DST_HOST_COUNTRY) memcpy(&d1->dst_ip_country, &s1->dst_ip_country, sizeof(d1->dst_ip_country)); 
  if (w2 & COUNT_DST_HOST_LAT) memcpy(&d1->dst_ip_lat, &s1->dst_ip_lat, sizeof(d1->dst_ip_lat)); 
  if (w2 & COUNT_DST_HOST_LONG) memcpy(&d1->dst_ip_long, &s1->dst_ip_long, sizeof(d1->dst_ip_long)); 
#endif
