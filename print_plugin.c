  ...
  for (j = 0; j < index; j++) {
    ...
    if (!go_to_pending) {
  	...
  	if (strlen(dst_host))
        ...
        if (config.what_to_count_2 & COUNT_SRC_HOST_LAT) fprintf(f, "%-5s       ", data->src_ip_lat.str);
        if (config.what_to_count_2 & COUNT_SRC_HOST_LONG) fprintf(f, "%-5s       ", data->src_ip_long.str);
        if (config.what_to_count_2 & COUNT_DST_HOST_COUNTRY) fprintf(f, "%-5s       ", data->dst_ip_country.str);
        if (config.what_to_count_2 & COUNT_DST_HOST_LAT) fprintf(f, "%-5s       ", data->dst_ip_lat.str);
        if (config.what_to_count_2 & COUNT_DST_HOST_LONG) fprintf(f, "%-5s       ", data->dst_ip_long.str);
  #endif
	...
	if (config.what_to_count_2 & COUNT_LABEL) P_fprintf_csv_label(f, pvlen, COUNT_INT_LABEL, write_sep(sep, &count), empty_string);
	  ...
	  if (strlen(pbgp->as_path))
        ...
        if (config.what_to_count_2 & COUNT_SRC_HOST_COUNTRY) fprintf(f, "%s%s", write_sep(sep, &count), data->src_ip_country.str);
	  	if (config.what_to_count_2 & COUNT_SRC_HOST_LAT) fprintf(f, "%s%s", write_sep(sep, &count), data->src_ip_lat.str);
	  	if (config.what_to_count_2 & COUNT_SRC_HOST_LONG) fprintf(f, "%s%s", write_sep(sep, &count), data->src_ip_long.str);
        if (config.what_to_count_2 & COUNT_DST_HOST_COUNTRY) fprintf(f, "%s%s", write_sep(sep, &count), data->dst_ip_country.str);
	  	if (config.what_to_count_2 & COUNT_DST_HOST_LAT) fprintf(f, "%s%s", write_sep(sep, &count), data->dst_ip_lat.str);
  		if (config.what_to_count_2 & COUNT_DST_HOST_LONG) fprintf(f, "%s%s", write_sep(sep, &count), data->dst_ip_long.str);
  #endif
...
void P_write_stats_header_formatted(FILE *f, int is_event)
{
  ...
  if (config.what_to_count_2 & COUNT_SRC_HOST_LAT) fprintf(f, "SH_LAT  ");
  if (config.what_to_count_2 & COUNT_SRC_HOST_LONG) fprintf(f, "SH_LONG  ");
  if (config.what_to_count_2 & COUNT_DST_HOST_COUNTRY) fprintf(f, "DH_COUNTRY  ");
  if (config.what_to_count_2 & COUNT_DST_HOST_LAT) fprintf(f, "DH_LAT  ");
  if (config.what_to_count_2 & COUNT_DST_HOST_LONG) fprintf(f, "DH_LONG  ");
#endif
...
void P_write_stats_header_csv(FILE *f, int is_event)
{
  ...
  if (config.what_to_count_2 & COUNT_SRC_HOST_LAT) fprintf(f, "%sSH_LAT", write_sep(sep, &count));
  if (config.what_to_count_2 & COUNT_SRC_HOST_LONG) fprintf(f, "%sSH_LONG", write_sep(sep, &count));
  if (config.what_to_count_2 & COUNT_DST_HOST_COUNTRY) fprintf(f, "%sDH_COUNTRY", write_sep(sep, &count));
  if (config.what_to_count_2 & COUNT_DST_HOST_LAT) fprintf(f, "%sDH_LAT", write_sep(sep, &count));
  if (config.what_to_count_2 & COUNT_DST_HOST_LONG) fprintf(f, "%sDH_LONG", write_sep(sep, &count));
#endif
