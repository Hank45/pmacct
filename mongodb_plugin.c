  ...
  for (j = 0, batch_idx = 0; j < index; j++) {
    ...
    if (!go_to_pending) {
	  ...
	  if (config.what_to_count_2 & COUNT_SRC_HOST_LAT) {
        if (strlen(data->src_ip_lat.str))
        bson_append_string(bson_elem, "ip_src_lat", data->src_ip_lat.str);
        else
        bson_append_null(bson_elem, "ip_src_lat");
      }
	  if (config.what_to_count_2 & COUNT_SRC_HOST_LONG) {
        if (strlen(data->src_ip_long.str))
        bson_append_string(bson_elem, "ip_src_long", data->src_ip_long.str);
        else
        bson_append_null(bson_elem, "ip_src_long");
      }
      if (config.what_to_count_2 & COUNT_DST_HOST_COUNTRY) {
	...
	if (config.what_to_count_2 & COUNT_DST_HOST_LAT) {
        if (strlen(data->dst_ip_lat.str))
        bson_append_string(bson_elem, "ip_dst_lat", data->dst_ip_lat.str);
        else
        bson_append_null(bson_elem, "ip_dst_lat");
      }

	if (config.what_to_count_2 & COUNT_DST_HOST_LONG) {
        if (strlen(data->dst_ip_long.str))
        bson_append_string(bson_elem, "ip_dst_long", data->dst_ip_long.str);
        else
        bson_append_null(bson_elem, "ip_dst_long");
      }
